...
public class Algorithms {
    ...
    public void executeSRT(List<Process> processList) {
        List<Process> completed = new LinkedList<>();
        List<Process> arrivedProcessesList = new LinkedList<>();
        ...
        arrivedProcessesList.get(0).setEndTime(arrivedProcessesList.get(0).getArrivalTime() + arrivedProcessesList.get(0).getServiceTime());
        int clock = 0;
        clock = arrivedProcessesList.get(0).getArrivalTime();

        boolean stop = !(processList.size() != 0 && arrivedProcessesList.size() != 0);
        while (!stop) {
            //check if a process has arrived
            if (processList.get(0).getArrivalTime() == clock) {
                arrivedProcessesList.add(processList.get(0));
                processList.remove(0);
            } else if (arrivedProcessesList.size() == 0 && currentProcess == null) {
                currentProcess = processList.get(0);
                processList.remove(currentProcess);

            }

            Process shortedProcessInQueue = arrivedProcessesList.stream().sorted(Comparator.comparing(Process::getRemainingTime)).findFirst().get();
            if (shortedProcessInQueue.getRemainingTime() < 0) {
    ...
    public Processes executeMLFM(List<Process> processList, int[] queueLength, int mode) {
    ...
    public Processes executeHRRN(List<Process> processList, int mode) {
        int clock = 0;
        Process temp;
        List<Process> completedProcessList = new LinkedList<>();
